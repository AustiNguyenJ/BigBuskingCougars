...
namespace Systems.SceneManagement {
    public class SceneGroupManager {
        ...
        public async Task UnloadScenes(SceneGroup sceneGroupToLoad = null) { 
            var scenes = new List<string>();
            var activeScene = SceneManager.GetActiveScene().name;
            
            string newActiveScene = sceneGroupToLoad?.FindSceneNameByType(SceneType.ActiveScene);
            bool replacingActiveScene = !string.IsNullOrEmpty(newActiveScene) && newActiveScene != activeScene;
            ...
            for (var i = sceneCount - 1; i > 0; i--) {
                ...
                if (sceneGroupToLoad != null && sceneGroupToLoad.SceneGroupContainsScene(sceneAt.name)) continue;
                
                var sceneName = sceneAt.name;
                
                if (sceneName == "Bootstrapper") continue;
                if (sceneName.Equals(activeScene) && !replacingActiveScene) continue;
        ...
        public async Task LoadDependencies(SceneGroup group, IProgress<float> progress) { 
            var loadedScenes = new List<string>();
            int sceneCount = SceneManager.sceneCount;
